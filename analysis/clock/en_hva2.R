...
select_top_cpgs <- function(X, Y, covars, n = 1000) {
  ...
  X <- t(X)
  ewas <- DML(X, ~ covars+Y, meta=metadf) # rows are probes and columns are samples
...
en <- cv.glmnet(x,y,type.measure='mse',family='gaussian',alpha=0.1,nfolds=3,parallel=TRUE,trace.it=1)
