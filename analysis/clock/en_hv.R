...
data$HV <- coldata$M_Hippo_VOL_ASHST1.combat
...
top_cpgs_corr <- select_top_cpgs_corr(X_train,Y_train,0.1)

top_temp <- intersect(top_cpgs_var,top_cpgs_corr)
top_cpgs <- intersect(top_cpgs_corr,ewas_cpgs)
# top_cpgs <- ewas_cpgs 
...
en <- cv.glmnet(x,y,type.measure='mse',family='gaussian',alpha=0.15,parallel=TRUE,trace.it=1)
...
y_pred_train <- predict(en,x,s=best_lambda.min)
...
y_pred_test <- predict(en, newx = x_test,s=best_lambda.min)
