...
ashs <- read.csv('/Users/lasyasreepada/Library/CloudStorage/Box-Box/ABC_MRI3T_ASHST1T2_measurements_20250425.csv')
...
# Select ADNI
spareba <- istaging[istaging$Study=='PENN',]
rm(istaging)
...
# Version 1: ICV only
model <- lm(M_Hippo_VOL_ASHST1 ~ ICV_ASHSICVatlas, data = abc)
coefficients <- coef(model)
abc$M_Hippo_VOL_ASHST1_Adj1 <- abc$M_Hippo_VOL_ASHST1 - (coefficients["ICV_ASHSICVatlas"] * (abc$ICV_ASHSICVatlas - mean(abc$ICV_ASHSICVatlas)))

# Version 2: Age and ICV
model <- lm(M_Hippo_VOL_ASHST1 ~ Age_MRI + ICV_ASHSICVatlas, data = abc)
coefficients <- coef(model)
abc$M_Hippo_VOL_ASHST1_Adj2 <- abc$M_Hippo_VOL_ASHST1 - (coefficients["ICV_ASHSICVatlas"] * (abc$ICV_ASHSICVatlas - mean(abc$ICV_ASHSICVatlas)) + coefficients["Age_MRI"] * (abc$Age_MRI - mean(abc$Age_MRI)))

# ADJUST THICKNESS

# Version 1: Age only
model <- lm(M_BA35_MeanTHK_MSTTHKMT_ASHST1_3T ~ Age_MRI, data = abc)
coefficients <- coef(model)
abc$M_BA35_MeanTHK_MSTTHKMT_ASHST1_Adj <- abc$M_BA35_MeanTHK_MSTTHKMT_ASHST1_3T - (coefficients["Age_MRI"] * (abc$Age_MRI - mean(abc$Age_MRI)))
...
# SAVE
library(arrow)
write_parquet(betas,'/Users/lasyasreepada/Projects/hippoage/data/INDD/20250530_abc_betas.parquet')
write.csv(coldata,'/Users/lasyasreepada/Projects/hippoage/data/INDD/20250530_abc_pheno.csv')
